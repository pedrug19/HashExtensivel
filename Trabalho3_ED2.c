...
//Marcação de lista para clientes
...
typedef struct disco {//controla a estrutura "principal" da Tabela Hash
    DIR *inicio;
    DIR *fim;
} DISCO;
...
LISTA *removeLista(LISTA *l, int c) {
    if (l->inicio == NULL) {
        printf("\nImpossível remover\n");
        return l;
    } else {
...
void salvaArquivo(LISTA *l) {
    ...
    CLI *aux = l->inicio;
    while (aux != NULL) {
        fprintf(arq, "%s:%d:%d:%f:%f:%d:%d\n", aux->nome, aux->conta, aux->CPF, aux->limite, aux->saldo, aux->valido,
                aux->linha);
...
DISCO *removeCliente(DISCO *d, LISTA *l) {//ARRUMAR
    //aqui também
    int CPF;
